...
plot.SampleSpecDiffFreq <- 
    function(x, method=NA, Kmax=4, pch=20, ...)
    {
        ...
        if(x$autok == 0){
            ...
            graphics::matplot(x$omega, pvals[,1:Kmax],  type='o', col=1, pch=pch, main=main, ...)
        } else {
            graphics::plot(x$omega, pvals,  type='o', col=1, pch=pch, main=main, ...)
        }
        rm(pvals)
    }
...
lines.SampleSpecDiffFreq <- 
    function(x, method=NA, Kmax=4, pch=20, ...)
    {
        ...
        if(x$autok == 0){
            Kmax=min( ncol(pvals), Kmax )
            graphics::matlines(x$omega, pvals[,1:Kmax],  type='o', pch=pch, main=main, ...)
        } else {
            graphics::lines(x$omega, pvals,  type='o',  pch=pch, main=main, ...)
        }
        rm(pvals)
    }
